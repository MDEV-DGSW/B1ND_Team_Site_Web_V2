...
const InquiryList = () => {
  ...
  const questionList = [
...

    {
      ...
      answer:
        '교내 학생들을 더욱 편리하게 관리할 수 있도록 스마트스쿨 플랫폼을 개발하고, 이외에도 대외활동을 이어나가고 있습니다.',
    },

    {
      ...
      answer:
        '평일에는 방과후 8,9교시와 야자 1,2교시까지 활동을하고, 주말에는 자유롭게 활동합니다.',
    },

    {
      ...
      answer:
        '바인드 공개 채용기간에 지원 할 수 있고, 여러 과정을 거쳐서 합격이되면 바인드팀에 입사 할 수 있습니다.',
    },

    {
      ...
      answer:
        '개인마다 1년에 4번 ~ 5번 사용이 가능하며, 개인적인 사정이있거나, 쉬고싶을때 언제든지 사용 가능합니다.',
    },

    {
      ...
      answer:
        '연말에 모든 바인드 팀원들이 모여서 다양한 얘기를 하며 1년동안의 활동을 되돌아봅니다',
    },
  ...
  const clickQuestionCard = useCallback(
    (id) => {
  ...
  const searchQuestions = (arrayParam) => {
    ...
    arrayParam = arrayParam.filter((contact) => {
      return contact.question.indexOf(searchValue) > -1;
    });
      ...
      return (
        <>
          <div className="InquiryList-ListBox-Contents-List" key={id}>
            Q.{' '}
            <span
              className="InquiryList-ListBox-Contents-List-Item"
              onClick={() => clickQuestionCard(id)}
            >
              {question}
            </span>
            {isAnswer && answerData.id === id && (
        ...
        <div className="InquiryList-TitleBox">
          <div className="InquiryList-TitleBox-Contents">
            <div className="InquiryList-TitleBox-Title">
              문의사항
              <button
                className="InquiryList-TitleBox-WriteButton"
                onClick={() =>
                  window.open('mailto:mdev_team@dgsw.hs.kr', '_blank')
                }
              >
                문의 하기
              </button>
            ...
            {/* <div className="InquiryList-TitleBox-SubTitle">
              ※ 계열사 지원문의는 각 회사의 채용 홈페이지에서 보다 빠르고 정확한 답변이 가능합니다. 같은 내용들 적을 공간입니다
            </div> */}
